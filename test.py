...
from components.layouts import update_global_statistics, plot_wordclouds
...
app.layout = html.Div([
    ...
    dbc.Container(    
                html.Div([
                    dbc.Row([
                        dbc.Col(html.Div('Unrecognised Wordcloud', className='text-primary')),
                        html.Img(id='unrecognised-wc'),
                        ]),
                    ],)
                ),
    dbc.Container(    
                html.Div([
                    dbc.Row([
                        dbc.Col(html.Div('Sometimes Recognised Wordcloud', className='text-primary')),
                        html.Img(id='part-recognised-wc'),
                        ]),
                    ],)
                ),
    dbc.Container(    
                html.Div([
                    dbc.Row([
                        dbc.Col(html.Div('Always Recognised Wordcloud', className='text-primary')),
                        html.Img(id='always-recognised-wc'),
                        ]),
                    ],)
                ),
    ] )
...
@app.callback(
    ...
    Output('unrecognised-wc', 'src'),
    Output('part-recognised-wc', 'src'),
    Output('always-recognised-wc', 'src')],
...
def update_div(duration_range, gender_list):
    ...
    return [update_global_statistics(global_stats, filtered_df, vocab_data, alphabet, metrics_available)] + plot_wordclouds(vocab_data)
